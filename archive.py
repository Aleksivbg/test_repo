...
print("Let's find an old website.")  # Выводим на экран приветственный текст нашей программы.
site = input("Type a website URL: ")  # Выводим на экран вопрос об URL, считываем пользовательский ввод и сохраняем это в переменной с именем site.
era = input("Type a year, month, and day, like 20150613: ")  # Выводим на экран еще один вопрос и на этот раз считываем год, месяц и день, а затем сохраняем их в переменной с именем  era.
...
try:  # Проверяем на ошибки: помещаем следующие четыре строки в блок try и, если находим ошибку, запускаем последнюю строку программы (она идет после ключевого слова except).
    ...
    print("Found this copy: ", old_site)  # Выводим на экран полученный URL.
    print("It should appear in your browser now.")  # Сообщаем о том, что случится, когда выполнится следующая строка.
    ...
    print("Sorry, no luck finding", site)  # Если программа дала сбой, выводим сообщение и имя сайта, который мы искали. Эта строка также имеет отступ, поскольку должна выполняться только в том случае, если выполняется строка except.
